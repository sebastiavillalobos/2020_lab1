...
(define (existeElemento lista elemento)
  (buscadorElementos lista elemento)
)

#|
; N es el contador, parte en 1
(define (buscaElemento lista elementoAComparar N)
  (if (equal? (getElemento lista N) elementoAComparar)
      #t
      (if (> (contadorLista lista) 0)
      (buscaElemento (cdr lista) elementoAComparar N )  
      #f
      )
  )
)
...
(define (buscadorElementos lista elemento)
  (if (> (contadorLista lista) 1)
    (if (equal? (car lista) elemento)
    #t
    (buscadorElementos (cdr lista) elemento)
    )
    (if (equal? (car lista) elemento)
    #t
    #f
...
(define (numeroElementosRepetidos lista elementoAComparar contador)
 ...
 ; Funcion que hace una lista con todos los elementos de una lista de una misma posicion

 (define (listaIguales lista posicion)
  (if (null? (cdr lista))
    (list (getElemento (car lista) posicion))
    (cons (getElemento (car lista) posicion) (listaIguales (cdr lista) posicion))
  )
 )
